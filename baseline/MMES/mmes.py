...
from baseline.MMES.es import ES  # abstract class of all Evolution Strategies (ES) classes
...
class MMES(ES):
    ...
    def iterate(self, x=None, mean=None, q=None, v=None, args=None):
        ...
        # Step 2: 检查终止条件并批量计算目标函数值
        if self._check_terminations():
            return x, y

        # 使用批量化目标函数评估
        y = self._evaluate_fitness(x, args)  # 假设 _evaluate_fitness 支持批量输入
