...
from baseline.JADE.de import DE  # abstract class of all differential evolution (DE)
...
class JADE(DE):
    ...
    def initialize(self, args=None):
        ...
        # --- [修改开始] ---
        # 假设 _evaluate_fitness 能够接收整个种群矩阵 x (n_individuals, ndim_problem)
        # 并返回一个包含所有适应度值的数组 y (n_individuals,)
    ...
    def select(self, args=None, x=None, y=None, x_cr=None, a=None, f_mu=None, p_cr=None):
            # --- [修改开始] ---
            # 1. 批量评估所有子代 x_cr 的适应度
            if self._check_terminations():
                yy_cr = np.full(self.n_individuals, np.inf)
            else:
                yy_cr = self._evaluate_fitness(x_cr, args) # 批量评估所有 n_individuals 个体
            
            # 2. 向量化比较和选择 (子代 yy_cr < 父代 y)
            successful_mask = yy_cr < y
            
            # 3. 更新种群 x 和适应度 y
            x[successful_mask] = x_cr[successful_mask]
            y[successful_mask] = yy_cr[successful_mask]
            
            # 4. 更新档案 a (档案中添加的是被淘汰的父代)
            # 注意：这里的 x 是被替换之前的父代个体，我们记录被子代替换掉的个体
            a = np.vstack((a, x[successful_mask]))
            
            # 5. 收集成功的自适应参数
            f = f_mu[successful_mask]
            p = p_cr[successful_mask]
            # --- [修改结束] ---
            
            # 6. 更新自适应参数 mu 和 median (保持不变)
            ...
            if len(f) != 0:  # for location update of Cauchy distribution
                # 注意：np.sum(np.power(f, 2)) / np.sum(f) 是 Lehmer mean (一种加权平均)
