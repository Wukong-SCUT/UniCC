...
from baseline.FCMAES.es import ES  # abstract class of all evolution strategies (ES)
...
class FCMAES(ES):
    ...
    <https://ieeexplore.ieee.org/abstract/document/8533604>`_
    IEEE Transactions on Cybernetics, 50(5), pp.2073-2083.

    Li, Z. and Zhang, Q., 2016.
    `What does the evolution path learn in CMA-ES?.
    <hhttps://link.springer.com/chapter/10.1007/978-3-319-45823-6_70>`_
    ...
    def iterate(self, mean=None, x=None, y=None, p=None, p_hat=None, args=None):
        ...
        # 批量生成基础 z 噪声
        z = self.rng_optimization.standard_normal((self.n_individuals, self.ndim_problem))  # shape: (n, d)

        # 如果处于初始阶段，使用 unbiased sampling
        if self._n_generations < self.m:
            x = mean + self.sigma * z  # broadcasting: mean shape (d,)
        else:
            # 生成两个额外的正态随机向量：每个个体一个标量
            eps1 = self.rng_optimization.standard_normal(self.n_individuals)  # shape: (n,)
            eps2 = self.rng_optimization.standard_normal(self.n_individuals)  # shape: (n,)

            # 注意：p_hat 是 (n, d)，p 是 (d,)

            # 计算各个项
            term1 = self._x_1 * z                           # shape: (n, d)
            term2 = (self._x_2 * eps1)[:, None] * p_hat     # shape: (n, d)
            term3 = (self._x_3 * eps2)[:, None] * p         # shape: (n, d)

            x = mean + self.sigma * (term1 + term2 + term3)  # shape: (n, d)

        # 并行评估适应度
        y = self._evaluate_fitness(x, args)  # shape: (n,)
